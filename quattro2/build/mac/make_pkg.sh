...
# create component.plist, distribution.xml and shell scripts
...
pkgbuild --analyze --root "$PKG_DIR/$FILES" "$PKG_DIR/$COMPOPLIST"
/usr/libexec/PlistBuddy -c "Set 0:BundleIsRelocatable false" "$PKG_DIR/$COMPOPLIST"
...
pkgbuild --root "$PKG_DIR/$FILES" \
         --component-plist "$PKG_DIR/$COMPOPLIST" \
