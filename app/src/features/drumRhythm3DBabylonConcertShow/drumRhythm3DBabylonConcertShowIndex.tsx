...
import {
  ...
  excludeHighwayMeshesFromShowLights,
} from './stage/concertShowHighwayLighting';
import { createConcertShowStageScene, type ConcertShowStageHandles } from './stage/concertStageScene';
import {
  LOOK_AHEAD_SEC,
  SPEED,
  buildConcertHighway,
} from '../drumRhythm3DBabylonConcert/stage/concertHighway';
import { DrumKitHitReaction } from '../drumRhythm3DBabylonConcert/stage/drumKitHitReaction';
import { StylizedHitVfx, type HitVfxKind } from '../drumRhythm3DBabylonConcert/stage/stylizedHitVfx';
...
export const DrumRhythm3DBabylonConcertShowIndex = () => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const stage: ConcertShowStageHandles = await createConcertShowStageScene(canvas);
      if (disposed) {
        stage.dispose();
        return;
      }
      ...
      const highway = buildConcertHighway(scene, chart, stage.camera, {
        ambientMax: CONCERT_SHOW_PERF.highwayAmbientParticles,
        ambientEmitRate: CONCERT_SHOW_PERF.highwayAmbientEmitRate,
        burstCount: CONCERT_SHOW_PERF.highwayBurstParticles,
      });
      const { pool, spawnBurst, padMatsMap, padLitColors } = highway;

      const highwayMeshes = scene.meshes.filter((m) => m.name.startsWith('gh'));
      excludeHighwayMeshesFromShowLights(scene, highwayMeshes);
      const highwayFill = createHighwayOwnFillLight(scene, stage.camera, highwayMeshes);
      for (const m of highwayMeshes) {
        m.receiveShadows = false;
      }

      const drumReaction = new DrumKitHitReaction(laneAnchors);
      const hitVfx = new StylizedHitVfx(scene, {
        glowBlurKernel: CONCERT_SHOW_PERF.glowBlurKernel,
        maxSparks: CONCERT_SHOW_PERF.maxSparks,
      });
